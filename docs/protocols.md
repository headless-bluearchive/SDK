...
SDK 文档保留的是游戏里“查看页面状态”和少量需要明确确认的日常操作：登录、资源同步、学生/物品/装备/编队、好友/社团、活动/活动商店、总力战/大决战排行、小游戏大厅、邮箱、任务、咖啡厅、课程表、扫荡预设，以及招募页面状态。后续新增协议时也按这个游戏功能边界维护。

标记为 `✓` 的少量会改变账号状态的协议，对应游戏里实际点下按钮的操作，例如好友申请、制造领取、邮件领取、任务领奖、课程表执行等。SDK 调用时必须传 `confirm=True`，并默认先查询当前页面状态做前置校验。
