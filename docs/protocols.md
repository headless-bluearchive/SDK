...
SDK 文档保留游戏内的“查看页面状态”和少量需要明确确认的日常操作：登录、资源同步、学生/物品/装备/编队、好友/社团、活动/活动商店、总力战/大决战排行、小游戏大厅、邮箱、任务、咖啡厅、课程表、扫荡预设，以及招募页面状态。后续新增协议时也按这个游戏功能边界维护。

标记为 `✓` 的少量会改变账号状态的协议，对应游戏内实际点击按钮的操作，例如好友申请、制造领取、邮件领取、任务领奖、课程表执行等。SDK 调用时必须传 `confirm=True`，并默认先查询当前页面状态做前置校验。
...
| 完成学园交流会 | `SchoolDungeon_List` 查询 + `ContentSweep_Request` 扫荡（`sweep.request(content=8)`，ContentType.SchoolDungeon） | 可支持：对**已通关**关卡扫荡即可推进；仅首次通关需实战（不接） |
| 完成/扫荡主线·日常副本关卡（普通/困难图） | `Campaign_List`/`WeekDungeon_List` 查询 + `ContentSweep_Request`/`MultiSweep`（主线 `content=1` CampaignMainStage、日常副本 `content=3` WeekDungeon；普通/困难由 `stage_id` 区分） | 可支持：对**已通关**关卡扫荡（消耗扫荡券/AP，需 `confirm=True`） |
| 参加战术大赛 | `Arena` 模块已删除 | 不支持（竞技场不接） |

说明：SDK 可以读取每日任务进度、领取已完成奖励，并对**已通关关卡执行扫荡**（学园交流会 / 主线普通·困难图 / 日常副本，`confirm=True`，消耗扫荡券或 AP）来推进对应每日/每周任务——扫荡属于独立动作，不在战斗边界内。仅**首次通关的实战、战斗结算、成绩提交、竞技场**等会影响公平性的链路不接。扫荡前提是该关卡已通关（困难图通常需 3 星解锁扫荡）；关卡 `stage_id` 来自 `campaign.list()` / `week_dungeon.list()` / `school_dungeon.list()`，取值见 [扫荡页](wiki/player/sweep.md) 与 [参数取值参考](wiki/reference.md)。
...
| 附件与头像框 | `Attachment_EmblemAcquire` | `46002` | 附件与头像框：领取已解锁 Emblem | 中 | ✓ | [Attachment](protocols/Attachment.md) |
| 附件与头像框 | `Attachment_EmblemAttach` | `46003` | 附件与头像框：装配 Emblem 到资料页 | 中 | ✓ | [Attachment](protocols/Attachment.md) |
...
| 战斗通行证 | `BattlePass_Check` | `51006` | 战斗通行证：检查领取状态 | 中 | ✓ | [BattlePass](protocols/BattlePass.md) |
| 充值与购买状态 | `Billing_TransactionStartByYostar` | `29000` | 充值与购买状态页相关协议：支付流程起点 | 高 | -  | [Billing](protocols/Billing.md) |
| 充值与购买状态 | `Billing_TransactionEndByYostar` | `29001` | 充值与购买状态页相关协议：支付流程终点 | 高 | -  | [Billing](protocols/Billing.md) |
| 充值与购买状态 | `Billing_PurchaseListByYostar` | `29002` | 充值与购买状态页相关协议：购买列表同步 | 高 | -  | [Billing](protocols/Billing.md) |
| 充值与购买状态 | `Billing_PurchaseCashShopVerifyByNexon` | `29003` | 充值与购买状态页相关协议：Nexon 购买校验 | 高 | -  | [Billing](protocols/Billing.md) |
| 充值与购买状态 | `Billing_CheckConditionCashShopGoods` | `29004` | 充值与购买状态页相关协议：条件商品检查 | 高 | -  | [Billing](protocols/Billing.md) |
| 充值与购买状态 | `Billing_PurchaseListByNexon` | `29005` | 充值与购买状态页相关协议：购买列表同步 | 高 | -  | [Billing](protocols/Billing.md) |
| 充值与购买状态 | `Billing_ValidateByNexon` | `29006` | 充值与购买状态页相关协议：支付校验 | 高 | -  | [Billing](protocols/Billing.md) |
| 充值与购买状态 | `Billing_FinishByNexon` | `29007` | 充值与购买状态页相关协议：支付收尾 | 高 | -  | [Billing](protocols/Billing.md) |
| 充值与购买状态 | `Billing_PurchaseFreeProduct` | `29008` | 充值与购买状态页相关协议：免费商品发放 | 高 | -  | [Billing](protocols/Billing.md) |
